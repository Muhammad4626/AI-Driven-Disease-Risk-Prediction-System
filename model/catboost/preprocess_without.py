...
# ---------------------------------
static_cols = [
    "Elevation_m", "River_Status",
    "Area_sq_km", "Sanitation_Index"
]
...
# ---------------------------------
# Calculate disease risk (ONLY to create target)
# ---------------------------------
...
# Remove last week per district (no next week available)
df = df.groupby('District', group_keys=False).apply(lambda x: x.iloc[:-1]).reset_index(drop=True)

# ---------------------------------
# Lag Features (Environmental ONLY)
# ---------------------------------
lag_features = [
    "Avg_rainfall", "Avg_Temperature", "Avg_Humidity",
    "Flood_Inundation", "Stagnant_Water", "Mean_NDVI"
...
# ---------------------------------
# Rolling Features (Environmental ONLY)
# ---------------------------------
for feature in lag_features:
    if feature in df.columns:
        df[f"{feature}_roll3"] = (
...
# ---------------------------------
# 🚨 DROP LEAKAGE COLUMNS (IMPORTANT)
# ---------------------------------
leakage_cols = [
    "Malaria_cases", "AD_cases", "Typhoid_cases",
...
# Save
# ---------------------------------
df.to_csv("processed_dataset_without.csv", index=False)
